...
export default function ScoutingForm({ onSubmit }: { onSubmit: Function }) {
    ...
    const submit = () => {
        if (formData.teamNumber && formData.drivetrain && formData.intakeCapabilities && formData.scoringLevels) {
            setFormData({
                teamNumber: 0,
            });
    ...
    const handleIntakeTypesChange = (name: string, checked: boolean) => {
        if (formData.intakeCapabilities && !checked) {
            setFormData({
                ...formData,
                intakeCapabilities: formData.intakeCapabilities.filter((i) => i !== name),
            });
    ...
    return (
        <>
            ...
            <div className='grid gap-6'>
                ...
                {errorMessage && <Label className='text-red-500 font-bold'>{errorMessage}</Label>}
